import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card"

interface OccupancyReportProps {
  data: { month: string; bookings: number }[]
}

export function OccupancyReport({ data }: OccupancyReportProps) {
  const totalBookings = data.reduce((sum, item) => sum + item.bookings, 0)
  const averageBookings = data.length > 0 ? totalBookings / data.length : 0

  return (
    <Card>
      <CardHeader>
        <CardTitle>Occupancy Report</CardTitle>
        <CardDescription>Monthly booking statistics</CardDescription>
      </CardHeader>
      <CardContent>
        <div className="space-y-4">
          <div>
            <p className="text-sm text-muted-foreground">Total Bookings</p>
            <p className="text-2xl font-bold">{totalBookings}</p>
          </div>
          <div>
            <p className="text-sm text-muted-foreground">Average Monthly Bookings</p>
            <p className="text-xl font-semibold">{averageBookings.toFixed(1)}</p>
          </div>
          <div className="space-y-2">
            <p className="text-sm font-medium">Monthly Breakdown</p>
            <div className="space-y-1">
              {data.slice(-6).map((item) => (
                <div key={item.month} className="flex justify-between text-sm">
                  <span>{item.month}</span>
                  <span className="font-medium">{item.bookings} bookings</span>
                </div>
              ))}
            </div>
          </div>
        </div>
      </CardContent>
    </Card>
  )
}
